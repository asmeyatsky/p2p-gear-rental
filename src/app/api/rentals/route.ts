  ...
  withMonitoring(
    withRateLimit(rateLimitConfig.general.limiter, rateLimitConfig.general.limit)(
      async (request: NextRequest) => {
        ...
        // Atomic conflict check + rental creation inside a serializable transaction
        // This prevents two concurrent bookings for overlapping dates
        const createdRental = await trackDatabaseQuery('rental.createWithConflictCheck', () =>
          prisma.$transaction(async (tx) => {
            // Check for booking conflicts inside the transaction
            const conflictingRentals = await tx.rental.findMany({
              where: {
                gearId,
                status: {
            ...
            if (conflictingRentals.length > 0) {
              logger.warn('Booking conflict detected', {
                ...
                conflictingRentals: conflictingRentals.map((r: any) => ({
                  id: r.id,
        ...
        return NextResponse.json({
          ...
          clientSecret: createdRental.clientSecret,
          priceBreakdown: {
            basePrice: priceBreakdown.basePrice,
            insuranceAmount: priceBreakdown.insuranceAmount,
            serviceFee: priceBreakdown.serviceFee,
